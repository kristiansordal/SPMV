...
void write_pid(int rank) {
    FILE *fp;

    if (rank == 0) {
        printf("Rank 0 truncating file\n");
        fp = fopen("/Users/kristiansordal/dev/uib/master/project/spmv/pids.txt", "w");
        fclose(fp);
    }

    printf("Rank %d waiting\n", rank);
    MPI_Barrier(MPI_COMM_WORLD);
    fp = fopen("/Users/kristiansordal/dev/uib/master/project/spmv/pids.txt", "a");
    fprintf(fp, "%d\n", getpid());
    fclose(fp);
    MPI_Barrier(MPI_COMM_WORLD);
}
...
int main(int argc, char *argv[]) {
    ...
    // overlap is now taken care of, can decrement num_rows
    M.num_rows--;
